...
vi.mock("../components/GenderField", () => ({ GenderField: "GenderField" }));
...
describe("profile form", () => {
  it("places the photo before name, gender and phone", async () => {
    ...
    const fields = view.root.findAll((n) => n.props.accessibilityLabel === "Add profile photo" || String(n.type) === "TextInput" || n.type === GenderField);
    expect(fields[0]!.props.accessibilityLabel).toBe("Add profile photo");
    expect(fields[1]!.props.accessibilityLabel).toBe("Name");
    expect(fields[2]!.type).toBe(GenderField);
  ...
  it("retains entered fields after upload failure and completes only after retry succeeds", async () => {
    ...
    await act(async () => { view.root.findByType(GenderField).props.onChange("man"); });
