...
const rows: Record<string, Row[]> = {
  ...
  sources: people.slice(0, 6).map((name, i) => ({
    id: `demo-source-${i}`,
    email: `${name.split(" ")[0]!.toLowerCase()}@example.com`,
  ...
  imports: people.slice(0, 4).map((name, i) => ({
    id: `demo-import-${i}`,
    email: `${name.split(" ")[0]!.toLowerCase()}@example.com`,
  ...
  feedback: [
    {
      id: "fb-1",
      message:
        "The share sheet does not show Allkept when I share from Safari. Works fine from Instagram.",
      email: "asha@example.com",
      app_version: "0.1.0 (24)",
      platform: "ios",
      created_at: ago(0),
      handled_at: null,
    },
    {
      id: "fb-2",
      message: "Love it. Could I get a widget for recent saves?",
      email: null,
      app_version: "0.1.0 (24)",
      platform: "android",
      created_at: ago(2),
