-- Search honours the intent filter, as it honours every other.
--
-- library_query_v4 let the Library be narrowed to what a save is for; without this, "To try" and
-- then a word typed into search would silently drop the narrowing, the very surprise v2 closed for
-- shapes and flags. Identical to v2 but for the one predicate, written exactly as library_query_v4
-- writes it so the two can never disagree.
--
-- v2 stays: the deployed edge function calls it until the new one is rolled out, and a search that
-- errors is worse than one that ignores a filter for a minute.

create function public.search_library_v3(
...
revoke all on function public.search_library_v3(text,extensions.vector,text[],text[],text[],text[],text[],jsonb,int) from public,anon;
grant execute on function public.search_library_v3(text,extensions.vector,text[],text[],text[],text[],text[],jsonb,int) to authenticated;
