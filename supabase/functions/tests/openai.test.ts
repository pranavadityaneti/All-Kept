...
Deno.test("openai: a picture travels as a high-detail image part before the words — a sign is unreadable at low detail; without one the input is the words alone", async () => {
  const seen: { url?: string; init?: RequestInit } = {};
  ...
  const body = JSON.parse(String(seen.init?.body)) as Record<string, unknown>;
  assertEquals(body["input"], [{ role: "user", content: [{ type: "input_image", image_url: "data:image/jpeg;base64,AAAA", detail: "high" }, { type: "input_text", text: "the words" }] }]);
