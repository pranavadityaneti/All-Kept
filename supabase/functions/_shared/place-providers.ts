  ...
  return async function search(query: string): Promise<Candidate[]> {
    const res = await fetchFn("https://places.googleapis.com/v1/places:searchText", {
      ...
      headers: {
        ...
        "X-Goog-FieldMask": "places.id,places.displayName,places.formattedAddress,places.location,places.primaryType,places.businessStatus,places.regularOpeningHours.weekdayDescriptions,places.googleMapsUri",
      },
